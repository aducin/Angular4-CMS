...
export class OrderService {
  ...
  headers: Headers;
	...
	message: any = null;
	request: any = null;
	constructor(private http:Http, private httpClient: HttpClient, private mainData: MainData) {
	...
	checkVouchers(id, token) {
		let url = this.mainData.url + 'customer/old/' + id + '/vouchers/' + token;
		return this.http.get(url)
			.map(res => res.json());
	}

	evenOrder(db, id, token) {
		let url = this.mainData.url + 'orders/' + db + '/' + id + '/even';
		let data = { tokn: token };
  	...
  	getOrder(db, id, token, additional = null) {
		var url = this.mainData.url + 'orders/' + db + '/' + id + '/' + token;
		if (additional === 'basic') {
			url = url + '?basic=true';
	...
	sendMail(data) {
		...
		var url = this.mainData.url + 'orders/' + data.db + '/' + data.id + '/mail/' + data.token;
